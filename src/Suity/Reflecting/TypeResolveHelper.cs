...
/// <summary>
/// Provides methods for resolving types and members by their string identifiers.
/// </summary>
public static class TypeResolveHelper
{
    private static readonly ConcurrentDictionary<string, Type> _typeResolveCache = new();
    private static readonly ConcurrentDictionary<string, MemberInfo> _memberResolveCache = new();

    private static ConcurrentBag<string> _unsolvedTypes = [];
    ...
    public static void CleanUpUnsolvedTypes()
    {
        _unsolvedTypes = [];
    }
    ...
    private static Type ResolveType(string typeString, IEnumerable<Assembly> searchAsm, MethodInfo declaringMethod)
    {
        ...
        if (_typeResolveCache.TryGetValue(typeString, out Type result))
        {
            return result;
        }

        if (_unsolvedTypes.Contains(typeString))
        {
            return null;
        }
        ...
        {
            _typeResolveCache[typeString] = result;
        }
        else
        {
            // If no result is found, also cache empty results to avoid increasing computation overhead.
            _unsolvedTypes.Add(typeString);
        }
    ...
    {
        if (_memberResolveCache.TryGetValue(memberString, out MemberInfo result))
        {
            return result;
        }
        ...
        if (result != null)
        {
            _memberResolveCache[memberString] = result;
        }
    ...
    private static Type FindTypeByCSCode(string csCodeType, IEnumerable<Assembly> asmSearch, Type declaringType = null)
    {
        ...
        if (csCodeType.IndexOfAny(['<', '>']) != -1)
        {
            ...
            string[] tokenTemp =
            [
                csCodeType.Substring(0, first),
                csCodeType.Substring(first + 1, last - (first + 1)),
                csCodeType.Substring(last + 1, csCodeType.Length - (last + 1)),
            ];
    ...
    public static void ResetCache()
    {
        ...
        _memberResolveCache.Clear();
    }
}

/// <summary>
/// Event args for member resolution events.
/// </summary>
public class ResolveMemberEventArgs(string memberId) : EventArgs
{
    private readonly string memberId = memberId;
