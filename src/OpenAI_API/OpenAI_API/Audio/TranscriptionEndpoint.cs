...
namespace OpenAI_API.Audio
{
	...
	public class TranscriptionEndpoint : EndpointBase, ITranscriptionEndpoint
	{
		...
		public async Task<string> GetTextAsync(Stream audioStream, string filename, string language = null, string prompt = null, double? temperature = null, CancellationToken cancel = default)
			=> await GetAsFormatAsync(audioStream, filename, AudioRequest.ResponseFormats.Text, language, prompt, temperature, cancel);
		...
		/// <param name="temperature">The sampling temperature, between 0 and 1. Higher values like 0.8 will make the output more random, while lower values like 0.2 will make it more focused and deterministic. If set to 0, the model will use log probability to automatically increase the temperature until certain thresholds are hit.</param>
		/// <returns>A string of the transcribed text</returns>
		public async Task<string> GetTextAsync(string audioFilePath, string language = null, string prompt = null, double? temperature = null, CancellationToken cancel = default)
		{
			using (var fileStream = File.OpenRead(audioFilePath))
			{
				return await GetTextAsync(fileStream, Path.GetFileName(audioFilePath), language, prompt, temperature, cancel);
			}
		}
		...
		/// <param name="temperature">The sampling temperature, between 0 and 1. Higher values like 0.8 will make the output more random, while lower values like 0.2 will make it more focused and deterministic. If set to 0, the model will use log probability to automatically increase the temperature until certain thresholds are hit.</param>
		/// <returns>A string of the transcribed text</returns>
		public async Task<AudioResultVerbose> GetWithDetailsAsync(Stream audioStream, string filename, string language = null, string prompt = null, double? temperature = null, CancellationToken cancel = default)
		{
			...
			return await HttpPost<AudioResultVerbose>(Url, content, cancel:cancel);
		}
		...
		/// <param name="temperature">The sampling temperature, between 0 and 1. Higher values like 0.8 will make the output more random, while lower values like 0.2 will make it more focused and deterministic. If set to 0, the model will use log probability to automatically increase the temperature until certain thresholds are hit.</param>
		/// <returns>A string of the transcribed text</returns>
		public async Task<AudioResultVerbose> GetWithDetailsAsync(string audioFilePath, string language = null, string prompt = null, double? temperature = null, CancellationToken cancel = default)
		{
			using (var fileStream = File.OpenRead(audioFilePath))
			{
				return await GetWithDetailsAsync(fileStream, Path.GetFileName(audioFilePath), language, prompt, temperature, cancel);
			}
		}
		...
		/// <param name="temperature">The sampling temperature, between 0 and 1. Higher values like 0.8 will make the output more random, while lower values like 0.2 will make it more focused and deterministic. If set to 0, the model will use log probability to automatically increase the temperature until certain thresholds are hit.</param>
		/// <returns>A string of the transcribed text</returns>
		public async Task<string> GetAsFormatAsync(Stream audioStream, string filename, string responseFormat, string language = null, string prompt = null, double? temperature = null, CancellationToken cancel = default)
		{
			var request = new AudioRequest()
			{
				Language = language ?? DefaultRequestArgs.Language,
			...
			return await HttpGetContent(Url, HttpMethod.Post, content, cancel);
		}
		...
		/// <param name="temperature">The sampling temperature, between 0 and 1. Higher values like 0.8 will make the output more random, while lower values like 0.2 will make it more focused and deterministic. If set to 0, the model will use log probability to automatically increase the temperature until certain thresholds are hit.</param>
		/// <returns>A string of the transcribed text</returns>
		public async Task<string> GetAsFormatAsync(string audioFilePath, string responseFormat, string language = null, string prompt = null, double? temperature = null, CancellationToken cancel = default)
		{
            using (var fileStream = File.OpenRead(audioFilePath))
			{
				return await GetAsFormatAsync(fileStream, Path.GetFileName(audioFilePath), responseFormat, language, prompt, temperature, cancel);
			}
		}
	}
}
