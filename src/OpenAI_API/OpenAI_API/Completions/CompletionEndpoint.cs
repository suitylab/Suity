...
namespace OpenAI_API.Completions
{
	...
	public class CompletionEndpoint : EndpointBase, ICompletionEndpoint
	{
		...
		public async Task<CompletionResult> CreateCompletionAsync(CompletionRequest request, CancellationToken cancel = default)
		{
			return await HttpPost<CompletionResult>(postData: request, cancel:cancel);
		}
		...
		public Task<CompletionResult> CreateCompletionsAsync(CompletionRequest request, int numOutputs = 5, CancellationToken cancel = default)
		{
			request.NumChoicesPerPrompt = numOutputs;
			return CreateCompletionAsync(request, cancel);
		}
		...
		public async Task StreamCompletionAsync(CompletionRequest request, Action<int, CompletionResult> resultHandler, CancellationToken cancel = default)
		{
			int index = 0;

			await foreach (var res in StreamCompletionEnumerableAsync(request, cancel))
			{
		...
		public async Task StreamCompletionAsync(CompletionRequest request, Action<CompletionResult> resultHandler, CancellationToken cancel)
		{
			await foreach (var res in StreamCompletionEnumerableAsync(request, cancel))
			{
				resultHandler(res);
		...
		public IAsyncEnumerable<CompletionResult> StreamCompletionEnumerableAsync(CompletionRequest request, CancellationToken cancel = default)
		{
			request = new CompletionRequest(request) { Stream = true };
			return HttpStreamingRequest<CompletionResult>(Url, HttpMethod.Post, request, cancel);
		}
