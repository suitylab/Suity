...
namespace OpenAI_API
{
	...
	public abstract class EndpointBase
	{
        ...
        /// <summary>
        /// The last sent request string
        /// </summary>
        public string LastRequestString { get; private set; }

        /// <summary>
        /// The last received response string
        /// </summary>
        public string LastResponseString { get; private set; }
		...
		protected HttpClient GetClient()
		{
			...
			// Set a longer timeout for AI interaction
		...
		private async Task<HttpResponseMessage> HttpRequestRaw(string url = null, HttpMethod verb = null, object postData = null, bool streaming = false, CancellationToken cancel = default)
		{
			...
			if (postData != null)
			{
				...
				else
				{
                    ...
                    string jsonContent = JsonConvert.SerializeObject(postData, jsonSetting);
					var stringContent = new StringContent(jsonContent, UnicodeEncoding.UTF8, "application/json");
					req.Content = stringContent;

					LastRequestString = jsonContent;
					LastResponseString = null;
                }
			}
			response = await client.SendAsync(req, streaming ? HttpCompletionOption.ResponseHeadersRead : HttpCompletionOption.ResponseContentRead, cancel);
			...
			else
			{
				...
				LastResponseString = resultAsString;
		...
		/// <exception cref="HttpRequestException">Throws an exception if a non-success HTTP response was returned</exception>
		internal async Task<string> HttpGetContent(string url = null, HttpMethod verb = null, object postData = null, CancellationToken cancel = default)
		{
			var response = await HttpRequestRaw(url, verb, postData, cancel:cancel);
		...
		/// <exception cref="HttpRequestException">Throws an exception if a non-success HTTP response was returned</exception>
		internal async Task<Stream> HttpRequest(string url = null, HttpMethod verb = null, object postData = null, CancellationToken cancel = default)
		{
			var response = await HttpRequestRaw(url, verb, postData, cancel:cancel);
		...
		private async Task<T> HttpRequest<T>(string url = null, HttpMethod verb = null, object postData = null, CancellationToken cancel = default) where T : ApiResultBase
		{
			var response = await HttpRequestRaw(url, verb, postData, cancel:cancel);
		...
		*/
		...
		/// <returns>An awaitable Task with the parsed result of type <typeparamref name="T"/></returns>
		/// <exception cref="HttpRequestException">Throws an exception if a non-success HTTP response was returned or if the result couldn't be parsed.</exception>
		internal async Task<T> HttpGet<T>(string url = null, CancellationToken cancel = default) where T : ApiResultBase
		{
			return await HttpRequest<T>(url, HttpMethod.Get, cancel:cancel);
		}
		...
		/// <returns>An awaitable Task with the parsed result of type <typeparamref name="T"/></returns>
		/// <exception cref="HttpRequestException">Throws an exception if a non-success HTTP response was returned or if the result couldn't be parsed.</exception>
		internal async Task<T> HttpPost<T>(string url = null, object postData = null, CancellationToken cancel = default) where T : ApiResultBase
		{
			return await HttpRequest<T>(url, HttpMethod.Post, postData, cancel);
		}
		...
		/// <returns>An awaitable Task with the parsed result of type <typeparamref name="T"/></returns>
		/// <exception cref="HttpRequestException">Throws an exception if a non-success HTTP response was returned or if the result couldn't be parsed.</exception>
		internal async Task<T> HttpDelete<T>(string url = null, object postData = null, CancellationToken cancel = default) where T : ApiResultBase
		{
			return await HttpRequest<T>(url, HttpMethod.Delete, postData, cancel);
		}
		...
		/// <returns>An awaitable Task with the parsed result of type <typeparamref name="T"/></returns>
		/// <exception cref="HttpRequestException">Throws an exception if a non-success HTTP response was returned or if the result couldn't be parsed.</exception>
		internal async Task<T> HttpPut<T>(string url = null, object postData = null, CancellationToken cancel = default) where T : ApiResultBase
		{
			return await HttpRequest<T>(url, HttpMethod.Put, postData, cancel);
		}
		...
		*/
		...
		/// <returns>The HttpResponseMessage of the response, which is confirmed to be successful.</returns>
		/// <exception cref="HttpRequestException">Throws an exception if a non-success HTTP response was returned</exception>
		protected async IAsyncEnumerable<T> HttpStreamingRequest<T>(string url = null, HttpMethod verb = null, object postData = null, CancellationToken cancel = default) where T : ApiResultBase
		{
			var response = await HttpRequestRaw(url, verb, postData, true, cancel:cancel);
