...
namespace OpenAI_API.Chat
{
	...
	public class ChatEndpoint : EndpointBase, IChatEndpoint
	{
		...
		public async Task<ChatResult> CreateChatCompletionAsync(ChatRequest request, CancellationToken cancel)
		{
			return await HttpPost<ChatResult>(postData: request, cancel: cancel);
		}
		...
		/// <returns>Asynchronously returns the completion result. Look in its <see cref="ChatResult.Choices"/> property for the results.</returns>
		public Task<ChatResult> CreateChatCompletionAsync(ChatRequest request, int numOutputs = 5, CancellationToken cancel = default)
		{
			request.NumChoicesPerMessage = numOutputs;
			return CreateChatCompletionAsync(request, cancel);
		}
		...
		public IAsyncEnumerable<ChatResult> StreamChatEnumerableAsync(ChatRequest request, CancellationToken cancel = default)
		{
			request = new ChatRequest(request) { Stream = true };
			return HttpStreamingRequest<ChatResult>(Url, HttpMethod.Post, request, cancel);
		}
