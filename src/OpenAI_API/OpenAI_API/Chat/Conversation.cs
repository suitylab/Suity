...
namespace OpenAI_API.Chat
{
    ...
    public class Conversation
    {
        ...
        /// Functions
        /// </summary>
        public IList<Function> Functions => _Functions;
        ...
        /// Tools
        /// </summary>
        public IList<Tool> Tools => _Tools;

        private List<Tool> _Tools; // Supports SiliconFlow
        ...
        /// <summary>
        /// Mod: Support Json return, accept <see cref="JsonSchema"/> or string
        /// </summary>
        ...
        /// </summary>
        public string LastRequestString => _endpoint?.LastRequestString;

        /// <summary>
        /// The last received response string
        /// </summary>
        public string LastResponseString => _endpoint?.LastResponseString;
        ...
        /// <param name="function"></param>
        /// <exception cref="ArgumentNullException"></exception>
        public void AppendTool(Function function) 
        {
            if (function is null)
            {
                throw new ArgumentNullException(nameof(function));
        ...
        public async Task<string> GetResponseFromChatbotAsync(CancellationToken cancel = default)
        {
            try
            {
                ChatRequest req = new ChatRequest(RequestParameters);
                req.Messages = _Messages.ToList();
                req.Functions = _Functions?.ToList();
                req.FunctionCall = FunctionCall;
                req.Tools = _Tools?.ToList();
                if (this.Schema != null)
                {
                    req.ResponseFormat = "json_schema"; // json_schema"
                    req.Schema = this.Schema;
                }

                var res = await _endpoint.CreateChatCompletionAsync(req, cancel);
        ...
        public async IAsyncEnumerable<string> StreamResponseEnumerableFromChatbotAsync(CancellationToken cancel = default)
        {
            ...
            while (retrying)
            {
                ...
                try
                {
                    resStream = _endpoint.StreamChatEnumerableAsync(req, cancel);
                    enumerator = resStream.GetAsyncEnumerator(cancel);
