...
public static class SubFlowUtility
{
    ...
    public static void SetScratchPad(this IAigcWorkflowPage page, ScratchPadTypes type, string path, string content = null, string note = null)
    {
        path = path?.Trim();
        if (string.IsNullOrWhiteSpace(path))
        {
        ...
        page.SetScratchPad(type, path, content, note);
    }

    public static void RemoveScratchPad(this IAigcWorkflowPage page, string path) 
        => SetScratchPad(page, ScratchPadTypes.Removed, path);
}
