    ...
    IDrawEditorImGui
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="AigcTaskPage"/> class.
    /// </summary>
    public AigcTaskPage()
    {
    ...
    public string GetPrompt(bool inHierarchy)
    {
        if (inHierarchy)
        {
            ...
            while (task != null)
            {
                ...
                task = task.ParentNode as AigcTaskPage;
            }
    ...
    private string GetLatestPrompt()
    {
        ...
        {
            if (list.GetItemAt(i) is not AigcTaskPage task)
            {
                continue;
            }
...
/*        if (string.IsNullOrWhiteSpace(taskPrompt))
    ...
    {
        if (_instance?.UseParentArticle == true && this.ParentNode is AigcTaskPage parent)
        {
    ...
    public IAigcTaskPage GetLastSubTask()
    {
        if (Count > 0)
        {
            return GetItemAt(Count - 1) as AigcTaskPage;
        }
        else
        {
            return null;
    ...
    {
        return Items.OfType<AigcTaskPage>()
            .SkipNull()
            .ToArray();
    }
    ...
    private void CollectChatHistory(LinkedList<LLmMessage> list, bool inHierarchy, int maxHistory, bool includeSelf)
    {
        ...
        if (index >= 0)
        {
            ...
            {
                if (ParentList.GetItemAt(i) is AigcTaskPage task)
                {
        ...
        if (inHierarchy && ParentNode is AigcTaskPage parent)
        {
    ...
    private IAigcTaskPage GetParentLastSubTask()
    {
        ...
        if (node.Count > 0)
        {
            return  node.GetItemAt(Count - 1) as AigcTaskPage;
        }
        else
        {
            return null;
    ...
    /// <summary>
    /// Creates a new task page from an <see cref="ISubFlowDef"/> definition.
    /// </summary>
    ...
    public AigcTaskPage CreateTaskPage(ISubFlowDef page, string title = null, string taskPrompt = null, string commitName = null)
    {
