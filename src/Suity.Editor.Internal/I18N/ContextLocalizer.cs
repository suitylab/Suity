    ...
    public class ContextLocalizer : ILocalizer
    {
        ...
        private protected void Load( XElement element )
        {
            foreach( var childElement in element.Elements() )
            {
                ...
                {
                    EditorServices.SystemLog.AddLog($"Failed to load localization entry: {err.Message}");
                }
            }
        }
        ...
        private void LoadEntry( XElement element )
        {
            ...
            foreach( var childElement in element.Elements() )
            {
                switch( childElement.Name.ToString() )
                {
                    case "Key":
                        ...
                        {
                            (keys ??= []).Add(UnescapeEscapeCodes(childElement.Value));
                        }
                        break;
            ...
            if (value != null)
            {
                ...
                //Suity.Editor.EditorServices.SystemLog.AddLog($"Add Entry: {key}");

                if( keys != null )
                {
                    foreach (var key2 in keys)
                    {
                        m_localizations[key2] = value;
                        //Suity.Editor.EditorServices.SystemLog.AddLog($"Add Ex-Entry: {key2}");
                    }
                }
            }
            else
            {
                //Suity.Editor.EditorServices.SystemLog.AddLog($"Skip Entry: {key}");
            }
        }
