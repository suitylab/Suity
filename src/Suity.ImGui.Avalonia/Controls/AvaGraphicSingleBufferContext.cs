...
/// </summary>
internal class AvaGraphicSingleBufferContext :
    IGraphicContext,
    IGraphicTextBoxEdit,
    ...
    IGraphicColorPicker
{
    ...
    public AvaGraphicSingleBufferContext(AvaSKGraphicControl control)
    {
    ...
    public void RequestOutput(IEnumerable<RectangleF> clipRects)
    {
        if (!_repaintAll && _supportDirtyRect)
        {
            ...
            var bounds = GetBoundingBox(_dirtyRects);
            if (bounds != RectangleF.Empty)
            {
                // Avalonia 11 supports partial redraw with Rect
                _control.Invalidate(new Avalonia.Rect(bounds.X, bounds.Y, bounds.Width, bounds.Height));
            }
            else
            {
                _control.Invalidate();
    ...
    {
        // Remove _offScreenLock and _bufferSurface release logic
        _output.InternalRelease();
    }

    /// <summary>
    /// Renders the graphic object directly to the specified canvas without double-buffering.
    /// </summary>
    ...
    public void Paint(ImmediateDrawingContext? context, SKCanvas canvas, Avalonia.Rect rect)
    {
        ...
        // Determine if it is a full redraw
        _output.RepaintAll = _repaintAll || !_supportDirtyRect || _dirtyRects == null;
        ...
        bool useClip = _supportDirtyRect && !_repaintAll && _dirtyRects != null;
        if (useClip)
        {
            canvas.Save();
            // Use SkiaSharp clipping with dirty region passed from Avalonia
            //canvas.SetClipRects(_dirtyRects);
            canvas.SetClipRect(rect.ToSystemDrawingRect());
        }

        try
        {
            // Directly update output context on target SKCanvas
