...
public class BSONValue
{
    ...
    #region Mod By Simage : Value Safe

    public Double doubleValueSafe
    {
        get
        {
            switch (mValueType)
    ...
    public DateTime dateTimeValueSafe
    {
        get
        {
            switch (mValueType)
...
public class BSONObject : BSONValue, IEnumerable
{
    ...
    {
        get { return mMap.GetValueSafe(key); }
        set { mMap[key] = value; }
    }
...
public sealed class SimpleBSON
{
    ...
    public static BSONObject Load(byte[] buf, int offset, int length)
    {
        SimpleBSON bson = new SimpleBSON(buf, offset, length);
    ...
    private SimpleBSON(byte[] buf, int offset, int length)
    {
        if (buf == null)
        {
        ...
        mMemoryStream = new MemoryStream(buf, offset, length);
        mBinaryReader = new BinaryReader(mMemoryStream);
    }
