...
# OBJECTIVE
Your task is to generate high-quality, executable source codes, based on the coding framework, the user request and tech-spec.
Focus on:
...
# BATCHING
You are in the batch generation process, there are files that are generated from previous batch in the chat history (if any).
## The current batch is listed below:
{{FILES}}

**Only output listed file in this batch**
...
# OUTPUT FORMAT RULES
- Only output files in current batch.
